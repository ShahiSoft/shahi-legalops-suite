...
class ModuleDashboard {
    ...
    public function __construct() {
        ...
        // Add AJAX handlers
        add_action('wp_ajax_shahi_toggle_module_premium', [$this, 'ajax_toggle_module']);
        add_action('wp_ajax_shahi_get_module_stats', [$this, 'ajax_get_module_stats']);
        add_action('wp_ajax_shahi_bulk_module_action', [$this, 'ajax_bulk_action']);
    }
    ...
    private function calculate_dashboard_stats($modules) {
        $total = count($modules);
        $active = count(array_filter($modules, function($m) { return $m['enabled']; }));
    ...
    private function get_module_usage_count($module_slug) {
        ...
        $counts = get_option('shahi_module_usage_counts', []);
    ...
    private function get_module_last_used($module_slug) {
        $last_used = get_option('shahi_module_last_used', []);
    ...
    private function get_module_dependencies($module_slug) {
        ...
        $dependencies = [
            'page-builder' => ['custom-post-types', 'widgets'],
            'seo-tools' => ['custom-post-types'],
            'social-sharing' => [],
        ];
        
        return isset($dependencies[$module_slug]) ? $dependencies[$module_slug] : [];
    }
    ...
    public function ajax_toggle_module() {
        ...
        if (empty($module_slug)) {
            wp_send_json_error(['message' => __('Invalid module.', 'shahi-legalops-suite')]);
        }
        ...
        if ($result) {
            ...
            wp_send_json_error(['message' => __('Failed to toggle module.', 'shahi-legalops-suite')]);
        }
    }
    ...
    public function ajax_bulk_action() {
        ...
        if (empty($action) || empty($modules)) {
            wp_send_json_error(['message' => __('Invalid bulk action.', 'shahi-legalops-suite')]);
        }
