...
class MissingFormLabelCheck extends AbstractCheck {
    ...
    public function check($content) {
        ...
        $elements = [];
        foreach ($inputs as $el) $elements[] = $el;
        foreach ($textareas as $el) $elements[] = $el;
        foreach ($selects as $el) $elements[] = $el;
        ...
        foreach ($elements as $element) {
            ...
            if ($tagName === 'input' && in_array($type, ['hidden', 'submit', 'button', 'image', 'reset'])) {
                continue;
            }
